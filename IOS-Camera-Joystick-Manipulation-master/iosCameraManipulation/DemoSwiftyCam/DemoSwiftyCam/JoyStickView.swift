...
//  DemoSwiftyCam
//
//  Created by Mohamed Bande on 10/7/17.
...
public final class JoyStickView: UIView {
	...
	*/
	public func resetFrame() {
		if (displacement < 0.5 || displacement > -0.5) && originalCenter != nil {
	...
	*/
	private func updateLocation(location: CGPoint) {
		...
		let yDelta = location.y - frame.mid.y
		
		// Calculate normalized displacement
		//
		let newDisplacement = yDelta / radius
		...
		if movable {
			if newDisplacement > 1.0 || newDisplacement < -1.0{
		...
		//
		let newClampedDisplacement: CGFloat!
		if(newDisplacement > 0){
			newClampedDisplacement = min(newDisplacement, 1.0)
		}
		else{
			newClampedDisplacement = max(newDisplacement, -1.0)
		}
