...
int main(int argc, char *argv[]){
	...
	if (stat("images", &st) == -1) {
	 ...
	 if(bool_imgtypefound == 1){ // Passive mode
		 printf("Chunk size is %d bytes. Image type is %s", chunk_size, img_type );
		 printf("===============================\n");
	 }
...
int chunk_count = 0;
	...
	printf("Dumping contents of catalog.csv", chunk_size);
	printf("===============================\n\n");
	while(1){
	...
	if (bool_imgtypefound){ // Passive mode only dl'd imgtype
		...
		for(int i = 0; files[i] != 0; i++){
			...
			while(1){
				int bytes_read = read(sockfd, buller, sizeof(buffer));
				if(bytes_read == 0){
