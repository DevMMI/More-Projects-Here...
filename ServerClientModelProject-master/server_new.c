...
int createCatalog(char *directory){
  ...
  while ((ent = readdir(dir)) != NULL){
    ...
    if(matched == 0){
      ...
      asprintf(&page,"\n%s, %i, %s", ent->d_name, lengthOfFile, checksum);
      DIR_COUNT++;
      fputs(page, f2);
...
int main(int argc, char *argv[])
{
 ...
 if (f == NULL) exit(EXIT_FAILURE);
 fputs("filename, size, checksum", f);
 fclose(f);
...
/* identify what mode client wants to be in....................*/
  ...
  if (strstr(mode, "passive")) {  // Passive mode..........................
    ...
    for(int i = 0; files[i] != 0; i++){
      ...
      //printf("check_sum %s\n", check_sum);
        ...
        while(1){
          ...
          bytes_read = read(fd, buller, sizeof(buffer));
          //printf("Bytes Read.....%d\n", bytes_read);
          if(bytes_read == -1){
            fprintf(stderr,"ERROR, bad file name\n");
            close(sockfd);
    ...
    while(1){
      ...
      printf("check_sum %s\n\n", check_sum);
      fflush(stdout);
      if (strstr(response, "0")) {  // if the client disconnects
        printf("Recieved disconnect command, closing socket\n");
        break;
      }
...
/*
FILE *t = fopen("images/dir0/1.jpg", "r");
int bytes_read = 0;
int fd = open("images/dir0/1.jpg", O_RDONLY);
while(1){
  bzero(buffer,256);
  bytes_read = read(fd, buffer, sizeof(buffer));
  if(bytes_read == -1){
